...
==============================================================================
ПІДСУМОК ТА АРХІТЕКТУРНІ ЗАМІТКИ: app/orchestrator.py
==============================================================================

Роль коду в системі:
"Оркеструє весь потік арбітражу" (Головний мозок / System Pipeline Orchestrator).

Призначення:
Управління життєвим циклом пошукових кампаній (навчання, фонове сканування, 
комплексний скоринг лотів, розрахунок чистяка та відправка сповіщень).

Ключові паттерни та рішення розробника:
1. Двоетапний життєвий цикл кампанії:
   - Phase 1 (Training): Збір 200 лотів для розрахунку базової ринкової ціни ($P_market$).
   - Phase 2 (Active Monitoring): Фоновий моніторинг тільки нових оголошень.
2. Потрійний бар'єр фільтрації угод (is_deal):
   Лот вважається угодою лише за умов: Score >= 70 AND Discount >= min_discount 
   AND Net Profit > 0 (з урахуванням логістики Нової Пошти та комісій OLX).
3. Рандомізація інтервалів запитів (Anti-Scraping Evasion):
   Використання `random.uniform(min, max)` у фоновому циклі для імітації людської поведінки.
4. Асинхронне винесення важких операцій:
   Використання `asyncio.to_thread` для обробки та збереження даних у БД, 
   щоб не блокувати головний Event Loop FastAPI.

Оцінка коду та покращення:
- Плюси: Повна автоматизація процесу арбітражу, прозора інтеграція всіх модулів.
- Мінуси: Небезпечний виклик `asyncio.run()` у синхронному контексті (`_run`),
  що може призвести до RuntimeError при виклику з асинхронних маршрутів.
"""
