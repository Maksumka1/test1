...
# ==============================================================================
# ПІДСУМОК ТА АРХІТЕКТУРНІ ЗАМІТКИ: app/analytics.py
# ==============================================================================
# Роль коду в системі:
# "Аналізує дані" (Аналітичний центр / Дашборд метрик).
#
# Призначення:
# Розрахунок стратегічних та операційних KPI для торгового дашборду (чистий прибуток,
# капітал, ROI угод, оборотність складських запасів, відсоток відхилення скаму).
#
# Ключові паттерни та рішення розробника:
# 1. Фінансова цілісність через бухгалтерські класи:
#    Прибуток рахується як різниця класів рахунків (Revenue - Expense) безпосередньо
#    з бухгалтерської книги (JournalLine/LedgerAccount).
# 2. Агрегація на боці БД (SQLAlchemy func):
#    Використання `func.coalesce(func.sum(...), 0.0)` запобігає поверненню NULL
#    та виконує масивні підрахунки прямо в PostgreSQL.
# 3. Торгові метрики арбітражу:
#    - Turnover Ratio (COGS / Inventory): оборотність товарних запасів.
#    - Avg Realization Days (30 / Turnover): середній час «заморожування» грошей у товарі.
#    - Return on Capital (Net Profit / Equity * 100%): окупність власного капіталу.
#
# Оцінка коду та покращення:
# - Плюси: Строга робота з UTC-часом, повний захист від ділення на нуль во всіх метриках.
# - Мінуси: Проблема N+1 у `average_deal_roi` (окремий SQL-запит всередині циклу
#   для кожного проданого товару). Потребує оптимізації через GROUP BY.
# ==============================================================================
