-- """
-- ==============================================================================
-- ПІДСУМОК ТА АРХІТЕКТУРНІ ЗАМІТКИ: db/schema.sql
-- ==============================================================================

-- Роль коду в системі:
-- "Визначає схему БД та тригер цілісності" (Database Schema & Integrity Enforcement Layer).

-- Призначення:
-- Структура таблиць PostgreSQL для подвійної бухгалтерії, складського інвентарю, 
-- пошукових кампаній, а також автоматична перевірка фінансового балансу через DB-тригер.

-- Ключові паттерни та рішення розробника:
-- 1. Відкладений тригер перевірки балансу (DEFERRABLE INITIALLY DEFERRED):
--    Тригер `trg_check_transaction_integrity` перевіряє рівність Sum(Debit) == Sum(Credit) 
--    не під час вставки кожного рядка, а в момент виконанні COMMIT. Це дозволяє 
--    пакетно додавати складні багаторядкові фінансові проводки.
-- 2. Захист даних на рівні БД (CHECK Constraints & NUMERIC):
--    Строгий контроль класів рахунків, статусів товарів та використання `NUMERIC(12, 2)` 
--    із застереженням `amount > 0.0` виключає появу невалідних грошових записів.
-- 3. UUID первинні ключі для транзакцій:
--    Генерація унікальних ідентифікаторів через `gen_random_uuid()` захищає від 
--    перебору транзакцій та забезпечує унікальність між системами.
-- 4. Ідемпотентне заповнення плану рахунків (Seed Data):
--    Використання `ON CONFLICT (code) DO NOTHING` дозволяє безпечно виконувати 
--    скрипт при кожному старті додатка без ризику дублювання рахунків.

-- Оцінка коду та покращення:
-- - Плюси: Еталонний архітектурний підхід до фінансових баз даних, наявність індексів.
-- - Мінуси: Тригер налаштовано тільки на `AFTER INSERT` (доцільно розширити до `AFTER UPDATE OR DELETE`).
-- ==============================================================================
-- """
-- ---------------------------------------------------------------------------
-- Chart of accounts
